...
async def wallet_present(request: Request):
    ...
    # 4) (Opcional) Enviar direct_post al verificador local
    sent = False
    post_status = None
    if do_send:
        try:
...
@router.post("/presentations/auto-from-authorize")
async def legacy_auto_from_authorize(request: Request):
    """
    Fallback que usa el front si /wallet/present no existe.
    Body:
    {
      "verifier_base": "https://...",
      "holder_did": "did:jwk:...",
    ...
    all_vcs = load_wallet_credentials(holder_did or "", password)
    if index < 0 or index >= len(all_vcs):
        return JSONResponse({"error": "Índice de VC inválido"}, status_code=400)

    chosen = [all_vcs[index]]
    priv = load_or_create_holder_key()
    did_local = holder_did or holder_did_from_key(priv)
    vp_jwt, vp_payload = make_vp_jwt(
        vc_jwts=chosen, iss_did=did_local, aud="openid://", nonce=auth["nonce"]
